        ...
        success: function (msg) {
            ...
            if (msg.status == 'success') {
                ...
                $.get("./templates/base/formstg_filter.template?v=8", function (data) {
                    ...
                    var month_y = get_date_x.getUTCMonth() + 1; //ambil bulan saat ini dalam numeric
                    ...
                    // kondisi jika mengambil tanggal akhir bulan sebelumnya, ketika sekarang tanggal 1
                    var filter_periode_data = '';
                    if(get_date_x.getDate()-1 !== 0){
                        filter_periode_data = year_y +'-'+ month_y +'-'+ (get_date_x.getDate()-1); //var periode data untuk next prosess
                    }else{
                        get_date_x = new Date(year_y,month_y-1,0);
                        filter_periode_data = year_y +'-'+ (month_y-1) +'-'+ get_date_x.getDate(); //var periode data untuk next prosess
                    }
                    

                    // set first value
                    var first_date = $('#periode_data').val(filter_periode_data);
                    ...
                    $('#periode_data').change(function(){
                        var get_periode = $('#periode_data').val(); //ambil nilai dari change
                        filter_periode_data = get_periode;
                        ...
                        if($('#periode_data').val() == ''){
                            ...
                            filter_periode_data = new Date($('#periode_data').val()); //ambil nilai saat ini
                            month_y = filter_periode_data.getUTCMonth(); //ambil bulan lalu dalam numeric
                            year_y = filter_periode_data.getFullYear();

                            // date_x = new Date(year_y,month_y+1,0);
                            filter_periode_data = year_y +'-'+ (month_y+1) +'-'+ filter_periode_data.getDate();
                    ...
                    //     if (args.column.field == "flag_validate" && args.data.flag_validate =='invalid')
                    //             $($(args.cell).parent()).css("backgroundColor", "yellow");/*custom css applied to the row */
                    // }
                    ...
                    }, //edit
                        {
                            ...
                            onInitializeForm: function (formid) {
                                ...
                                setTimeout(function () {
                                    ...
                                    var var_date = $('.input-group input#periode_data').val();

                                    $('.DataTD input#periode_data ').is('[readonly]') ?  $('.DataTD input#periode_data').val(var_date) : '';
                                    $('.DataTD input#periodeData ').is('[readonly]') ?  $('.DataTD input#periodeData').val(var_date) : '';
                                    ...
                                    $('.DataTD input#periodeData ').on('click',function(){
                                        $('.datepicker').show();
                                        $('.datepicker').removeClass('hidden');
                                    ...
                                    $('.DataTD input[readonly="readonly"]#periodeData ').on('click',function(){
                                        $('.datepicker').addClass('hidden');
                                        $('.datepicker').remove();
                                    });
                    ...
                    // scrol body grid
                    // $('.ui-jqgrid-bdiv').children('div:nth-child(1)').css({"max-height":"327px","height":"auto"});
                    // end scrol body grid
