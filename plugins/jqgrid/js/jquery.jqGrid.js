	...
	styleUI : {
		...
		Bootstrap : {
			...
			navigator : {
				...
				icon_help_nav : "glyphicon-question-sign",
...
$.jgrid.extend({
	...
	navGrid : function (elem, p, pEdit, pAdd, pDel, pSearch, pView, pHelp) { 
		...
		return this.each(function() {
			...
			for(i = 0; i<clone; i++) {
				...
				if(o.add || o.edit || o.del || o.view|| o.help) {$("tr",navtbl).append(sep);}
				pHelp = pHelp || {};
				if (o.help) {
					tbd = $("<td class='ui-pg-button "+commonstyle.cornerall+"'></td>");
					$(tbd).append("<div class='ui-pg-div'><span class='"+commonstyle.icon_base+" "+o.helpicon+"'></span></div>");
					$("tr",navtbl).append(tbd);
					$(tbd,navtbl)
					.attr({"title":o.helptitle || "",id: pHelp.id || "help_"+elemids})
					.click(function(){
						//alert(pHelp);
						window.open('./addons/pdfviewer/viewer.html?pdf='+pHelp.helppdffile+'#'+pHelp.helppage,'PDF Helps','directories=no,titlebar=no,toolbar=no,location=0,address=0,status=no,menubar=no,scrollbars=no,resizable=no,width=600,height=550');
						return false;
					}).hover(onHoverIn, onHoverOut);
					tbd = null;
				}
	...
	exportToExcel : function ( o ) {
		...
		this.each(function() {
			...
			for ( j=0, ien=cm.length ; j<ien ; j++ ) {
				...
				// 	continue;
				// }
				if(cm[j].name === 'cb' || cm[j].name === 'rn' || !cm[j].exportcol) {
					continue;
				}
